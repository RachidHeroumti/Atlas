...
<div class="relative flex flex-col">
    ...
    <div v-if="$settings.sections.products.add_to_wishlist.active">
        <button v-if="$store.state.wishlist.find(i=>i._id==item._id)"
             @click="removeFromWishlist" title="Wishlist" class="absolute z-10 p-2 m-2 mx-1 bg-gray-100 rounded-md item top-1 right-2 hover:bg-gray-200">
   ...
   <!--
    ...
    <div class="relative flex h-full ">
        ...
        class="flex flex-col w-full h-full  border shadow-md justify-center items-center  p-4"> 

            <div class="flex justify-center items-center w-full  ">
                <nuxt-link :to="`/products/${item.slug}`" :title="item.name" :aria-label="item.name">
                    <si-image   
                    class=" object-cover transform  h-56 w-56  m-2"
                    :class="{ '': !isHovering, 'scale-110 transition-transform duration-700': isHovering }"
            ...
            <div class="h-full w-full text-center">
                <nuxt-link :to="`/products/${item.slug}`">
                    <h3 class="m-2 font-poppins font-normal  text-base two-lines">{{ item.name }}</h3>
                </nuxt-link>
            </div>
                ...
                <div class="flex mb-1">
                    <span v-for="(star,i) in 5" :class="star <= item.review.rating ? 'text-yellow-500 ': 'text-gray-400'" :key="i">
                        <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="star" role="img" xmlns="http://www.w3.org/2000/svg"
                         viewBox="0 0 576 512" class="w-3 h-3 translate"><path fill="currentColor" d="M259.3 17.8L194 150.2 47.9 171.5c-26.2 3.8-36.7 36.1-17.7 54.6l105.7 103-25 145.5c-4.5 26.3 23.2 46 46.4 33.7L288 439.6l130.7 68.7c23.2 12.2 50.9-7.4 46.4-33.7l-25-145.5 105.7-103c19-18.5 8.5-50.8-17.7-54.6L382 150.2 316.7 17.8c-11.7-23.6-45.6-23.9-57.4 0z" class=""></path></svg>
                    </span>
                </div>
       ...
       <!-- Add to cart product-->
         <button v-else-if="$settings.sections.products.add_to_cart.active" @click="addToCart" 
          ...
          :class="{ 'invisible': !isHovering, 'visible': isHovering }"
            >
                <span class="">{{ $settings.sections.products.add_to_cart.text }} +</span>
            </button>
     </div>
