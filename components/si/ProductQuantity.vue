<template>
    <div class="flex items-center font-poppins font-base justify-center  text-gray-400 border border-gray-300 rounded text-sm ">
        <button @click="inc(-1)" class="  py-2 px-4" :class="value > quantity.min ? ' ' : ''">&lt;</button>

        <div class="flex">
            <label class="relative  py-2 h-full">
                <span class="px-2 py-4 text-center ">{{ value }}</span>
        ...
        <button @click="inc(1)" class=" py-2 px-4" :class="value < quantity.instock ? '' : ''">></button>
    </div>
</template>
