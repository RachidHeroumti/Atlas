...
<div class="  text-titles-color bg-white    w-full fixed top-0 left-0 right-0 h-20 z-30 border-b border-gray-200  "> <!--fixed top-0 left-0  z-50  -->
    ...
    <div class="relative  h-full">
        <div class=" flex items-center justify-between p-2 h-full   ">
            ...
            <button @click="$store.state.showHeaderMenu = !$store.state.showHeaderMenu" aria-label="Search button"
             class="flex sm:hidden flex-col p-2   ">
                    <si-svg><svg aria-hidden="true" focusable="false" data-prefix="fad" data-icon="bars" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"
                     class="w-6 h-6 translate text-gray-800"><g class="fa-group">
                        <path fill="currentColor" d="M16 288h416a16 16 0 0 0 16-16v-32a16 16 0 0 0-16-16H16a16 16 0 0 0-16 16v32a16 16 0 0 0 16 16z" 
                    class="fa-secondary">
              ...
              <div v-if="$store.state.showHeaderMenu" class="px-10 bg-white  sm:hidden fixed top-16 left-0 right-0 flex z-20 justify-between  w-screen">
                 <div class=" w-full"> 
                <ul v-for="(item, i) in menu.items" :key="i" class="flex flex-col w-full  ">
                    <li class="flex items-center mb-1 justify-between w-full   border-b-2  border-gray-200 hover:underline hover:underline-offset-4">
                        <router-link class="m-1 w-full flex font-poppins text-13p text-titles-color font-medium" :to="item.url">{{ item.text }}</router-link>
                    ...
                    <div v-if="item._id == activeId">
                        <div v-for="(item,i) in item.childrens" :key="i" class="bg-gray-100">
                            <router-link class="m-1 rounded-md  hover:bg-white hover:text-gray-300 flex" :to="item.url">{{item.text}}</router-link>
                            <ul class=" " v-if="item.childrens && item.childrens.length > 0">
                                <li v-for="(child,ii) in item.childrens" :key="ii">
                                    <nuxt-link  class=" m-1 rounded-md  hover:bg-white hover:text-gray-300 flex" :to="child.url">
                                        {{ child.text }}
                                    </nuxt-link>
                                </li>
                            </ul>
   ...
   <nuxt-img width="70" height="50" property="height" class="object-contain w-full h-12"
      ...
      <div class=" flex  sm:w-full text-dark-gray ">
  ...
  <ul v-for="(item, i) in menu.items" :key="i" class="flex flex-col  menuTextStyle cursor-pointer">
    ...
    >
      ...
      >
        <li v-for="(child, ii) in item.childrens" :key="ii" class=" py-1 hover:underline">
          <nuxt-link class="flex whitespace-nowrap font-base  text-12p " :to="child.url">
            {{ child.text }}
          </nuxt-link>
        </li>
      </ul>
               ...
               <div class="flex items-center justify-start  h-full ">
                ...
                <div v-if="$settings.sections.header.icons.currency && $settings.store_currencies.length > 1" class="relative mx-1">
                    <div v-if="iconMenu == 'currency'" class="absolute z-50 flex flex-col bg-gray-100 rounded-md top-5">
                        <a class="p-2 m-1  whitespace-nowrap" v-for="cur of $settings.store_currencies" :key="cur.code" :href="`?cur=${cur.code}`">{{ cur.name }} ( {{ cur.code }} )</a>
                    </div>
                </div>
                ...
                <div v-if="$settings.sections.header.icons.language && $settings.store_languages.length > 1" class="relative mx-1">
                    <div v-if="iconMenu == 'language'" class="absolute z-50 flex flex-col bg-gray-100 rounded-md top-5">
                        <a class="p-2 m-1  whitespace-nowrap" v-for="lang of $settings.store_languages" :key="lang.code" :href="`?lang=${lang.code}`">{{ lang.name }} ( {{ lang.code }} )</a>
                    </div>
                </div>
